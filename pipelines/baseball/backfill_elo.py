"""
Baseball ELO backfill pipeline.

Replays all finished baseball matches chronologically through BaseballEloEngine
and writes one rating_elo_team row per (team, match).

Idempotent: deletes existing baseball rating_elo_team rows before rebuilding.

Usage:
    python -m pipelines.baseball.backfill_elo
    python -m pipelines.baseball.backfill_elo --incremental
"""
...
from ratings.baseball_elo import BaseballEloEngine, COMPETITION_IMPORTANCE
...
def run_backfill(incremental: bool = False) -> int:
    ...
    try:
        matches = (
            ...
            .filter(CoreMatch.status == "finished", CoreMatch.sport == "baseball")
            .order_by(CoreMatch.kickoff_utc.asc())
            .all()
        )
        log.info("Found %d finished baseball matches to process.", len(matches))

        league_importance = build_league_importance_map(session, "baseball", COMPETITION_IMPORTANCE)
        log.info("Loaded importance multipliers for %d leagues.", len(league_importance))

        park_factors = load_park_factors(session)
        log.info("Computed park factors for %d teams.", len(park_factors))
        ...
        if not incremental:
            ...
            if sport_match_ids:
                ...
                log.info("Cleared %d existing baseball rating_elo_team rows.", deleted)
                session.flush()
        else:
            sport_match_ids = [m.id for m in matches]
            existing = (
        ...
        for match in matches:
            ...
            park_factor = park_factors.get(match.home_team_id, 1.0)
        ...
        session.commit()
        log.info("Baseball ELO backfill complete. %d rating rows written.", rows_written)
    ...
    log.info("Top 10 baseball teams by ELO:")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill baseball ELO ratings")
