"""
Baseball prediction pipeline — ML model with ELO fallback.

Usage:
    python -m pipelines.baseball.predict_baseball
    python -m pipelines.baseball.predict_baseball --match-id <uuid>
    python -m pipelines.baseball.predict_baseball --all
"""
...
from ratings.baseball_elo import BaseballEloEngine
try:
    from pipelines.baseball.feature_engineering import build_feature_vector
except ImportError:
...
def _load_elo_engine(session: Session) -> BaseballEloEngine:
...
def _predict_elo(match: CoreMatch, engine: BaseballEloEngine, session: Session) -> dict:
    ...
    context = MatchContext(
        match_id=match.id, sport=Sport.BASEBALL, date=kickoff,
...
def run_predictions(session, matches, payload, engine) -> int:
    ...
    for match in matches:
        try:
            data = _predict_ml(match, payload, session) if payload else _predict_elo(match, engine, session)
...
def run(match_id: Optional[str] = None, all_matches: bool = False) -> int:
    ...
    try:
        ...
        log.info("Running baseball predictions for %d matches (model=%s)...",
                 len(matches), payload["model_name"] if payload else "elo-v1")
        count = run_predictions(session, matches, payload, engine)
        session.commit()
        log.info("Baseball predictions complete. %d rows upserted.", count)
        return count
    except Exception:
        session.rollback()
