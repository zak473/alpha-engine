...
def _load_training_data(session) -> tuple[np.ndarray, np.ndarray, list[str]]:
    """
    Load finished CoreMatch rows for baseball where outcome is known.
    Computes feature vectors live using baseball feature_engineering.
    ...
    for match in rows:
        ...
        if label is None:
            log.warning(
                "Unknown outcome %r for match %s — skipping",
                match.outcome,
                match.id,
            )
            skipped += 1
    ...
    if skipped:
        log.info("Skipped %d matches due to missing outcome or feature errors.", skipped)
