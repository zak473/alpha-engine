"""
Hockey match outcome model — LightGBM version.

Replaces the LogisticRegression baseline (hockey_lr_v*) with LightGBM which
handles non-linear interactions between ELO, form, and rolling stats natively.
Binary classification: home_win (0) vs away_win (1) — NHL has no draws.

Algorithm:   LGBMClassifier (binary)
Evaluation:  Walk-forward split (train on oldest 80%, evaluate on newest 20%)
Output:      artefacts/hockey_lgb_v{n}.joblib + model_registry row

Usage:
    python -m pipelines.hockey.train_hockey_lgb
    python -m pipelines.hockey.train_hockey_lgb --version v2
"""
...
# ---------------------------------------------------------------------------
# Bulk data store (identical to train_hockey_model.py)
# ---------------------------------------------------------------------------
...
def _build_features_bulk(store: BulkDataStore, match: CoreMatch) -> list[float]:
    ...
    def rolling_stats(team_id: str) -> dict:
        ...
        goals, goals_allowed, shots, save_pct = [], [], [], []
        pp_pct, fo_pct, hits, pim = [], [], [], []
        ...
        for m in recent10:
            ...
            if own:
                ...
                if own.shots_on_goal is not None:      shots.append(float(own.shots_on_goal))
                if own.save_pct is not None:           save_pct.append(float(own.save_pct))
                ...
                if own.penalty_minutes is not None:    pim.append(float(own.penalty_minutes))
            else:
            ...
            if opp and opp.goals is not None:
                goals_allowed.append(float(opp.goals))
            else:
                opp_score = m.away_score if is_home else m.home_score
                if opp_score is not None:
                    goals_allowed.append(float(opp_score))

        return {
            "goals_avg":         _avg(goals),
            "goals_allowed_avg": _avg(goals_allowed),
            "shots_avg":         _avg(shots),
            "save_pct_avg":      _avg(save_pct),
            "pp_pct_avg":        _avg(pp_pct),
            "faceoff_pct_avg":   _avg(fo_pct),
            "hits_avg":          _avg(hits),
            "pim_avg":           _avg(pim),
        }
...
def train(version: Optional[str] = None) -> str:
    ...
    try:
        ...
        model_name = f"hockey_lgb_{version}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train hockey LightGBM prediction model")
