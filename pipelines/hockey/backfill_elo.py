"""
Hockey ELO backfill pipeline.

Replays all finished hockey matches chronologically through HockeyEloEngine
and writes one rating_elo_team row per (team, match).

Idempotent: deletes existing hockey rating_elo_team rows before rebuilding.

Usage:
    python -m pipelines.hockey.backfill_elo
    python -m pipelines.hockey.backfill_elo --incremental
"""
...
from ratings.hockey_elo import HockeyEloEngine, COMPETITION_IMPORTANCE
...
def run_backfill(incremental: bool = False) -> int:
    ...
    try:
        matches = (
            ...
            .filter(CoreMatch.status == "finished", CoreMatch.sport == "hockey")
            .order_by(CoreMatch.kickoff_utc.asc())
            .all()
        )
        log.info("Found %d finished hockey matches to process.", len(matches))

        league_importance = build_league_importance_map(session, "hockey", COMPETITION_IMPORTANCE)
        ...
        if not incremental:
            ...
            if sport_match_ids:
                ...
                log.info("Cleared %d existing hockey rating_elo_team rows.", deleted)
                session.flush()
        else:
            sport_match_ids = [m.id for m in matches]
            existing = (
        ...
        for match in matches:
            ...
            # Detect OT/SO: prefer stored period_type, fall back to current_period proxy
            pt = match.period_type
            if pt is None and match.current_period is not None:
                pt = "overtime" if match.current_period > 3 else "regulation"
            ...
            context = MatchContext(
                ...
                extra={"period_type": pt} if pt else {},
            )
        ...
        session.commit()
        log.info("Hockey ELO backfill complete. %d rating rows written.", rows_written)
    ...
    log.info("Top 10 hockey teams by ELO:")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backfill hockey ELO ratings")
