...
def _parse_sets(scores: list[dict]) -> tuple[Optional[int], Optional[int], str]:
    ...
    scores format: [{"score_first": "6", "score_second": "3", "score_set": "1"}, ...]
    """
    sets_detail = []
    a_sets = b_sets = 0
    for s in (scores or []):
        ...
        sets_detail.append({"set_num": len(sets_detail) + 1, "a": a_g, "b": b_g})
        if a_g > b_g:
            a_sets += 1
        elif b_g > a_g:
            b_sets += 1
...
def _upsert_match_stats(
    ...
    for side, player_id in [("first", home_team_id), ("second", away_team_id)]:
        ...
        row.break_points_faced = s.get("break_points_faced")
        row.break_points_saved = s.get("break_points_saved")
        row.break_points_created = s.get("break_points_created")
        row.break_points_converted = s.get("break_points_converted")
        row.bp_conversion_pct = s.get("bp_conversion_pct")
        ...
        for event in unique_events:
            try:
                ...
                if pointbypoint:
                    ...
                    serve_stats = _parse_serve_stats(pointbypoint, first_player, second_player)
...
def build_player_form(dry_run: bool = False) -> int:
    ...
    with Session(engine) as session:
        ...
        for player in players:
            # All finished tennis matches for this player
            matches = (
            ...
            for m in matches:
                ...
                # Win/loss
                if m.outcome == "H" and is_home:
                    wins += 1
                elif m.outcome == "A" and not is_home:
                    wins += 1
                else:
                    losses += 1
            ...
            form_row = (
                ...
                .filter_by(player_id=player.id, surface="all", window_days=365)
                .first()
            )
            if form_row is None:
            ...
            form_row.avg_service_hold_pct = round(sum(hold_pcts) / len(hold_pcts), 3) if hold_pcts else None
            form_row.avg_bp_conversion_pct = round(sum(bp_conv_pcts) / len(bp_conv_pcts), 3) if bp_conv_pcts else None
            updated += 1
