"""
Esports prediction pipeline — ML model with ELO fallback.

Usage:
    python -m pipelines.esports.predict_esports
    python -m pipelines.esports.predict_esports --match-id <uuid>
    python -m pipelines.esports.predict_esports --all
"""
...
from pipelines.esports.feature_engineering import build_feature_vector
...
def _load_elo_engine(session: Session) -> EsportsEloEngine:
    ...
    log.info("Loaded ELO ratings for %d esports team entries.", len(rows))
    return engine
...
def _predict_elo(match: CoreMatch, engine: EsportsEloEngine) -> dict:
    ...
    context = MatchContext(
        match_id=match.id, sport=Sport.ESPORTS, date=kickoff,
...
def run(match_id: Optional[str] = None, all_matches: bool = False) -> int:
    ...
    try:
        ...
        log.info("Running esports predictions for %d matches (model=%s)...",
                 len(matches), payload["model_name"] if payload else "elo-v1")
        count = run_predictions(session, matches, payload, engine)
        session.commit()
        log.info("Esports predictions complete. %d rows upserted.", count)
        return count
    except Exception:
        session.rollback()
