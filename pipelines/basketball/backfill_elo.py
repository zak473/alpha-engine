...
from ratings.basketball_elo import BasketballEloEngine, COMPETITION_IMPORTANCE
...
def run_backfill(incremental: bool = False) -> int:
    ...
    try:
        ...
        league_importance = build_league_importance_map(session, "basketball", COMPETITION_IMPORTANCE)
        ...
        for match in matches:
            ...
            # Detect OT from current_period (basketball is 4 quarters; >4 = OT)
            pt = match.period_type
            if pt is None and match.current_period is not None:
                pt = "overtime" if match.current_period > 4 else "regulation"
