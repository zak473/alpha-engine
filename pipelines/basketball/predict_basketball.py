"""
Basketball prediction pipeline — ML model with ELO fallback.
...
from pipelines.basketball.feature_engineering import build_feature_vector
...
def _load_elo_engine(session: Session) -> BasketballEloEngine:
    ...
    log.info("Loaded ELO ratings for %d basketball team entries.", len(rows))
    return engine
...
def _predict_elo(match: CoreMatch, engine: BasketballEloEngine) -> dict:
...
def run(match_id: Optional[str] = None, all_matches: bool = False) -> int:
    ...
    try:
        ...
        model_version = payload["model_name"] if payload else "basketball-elo-v1"
