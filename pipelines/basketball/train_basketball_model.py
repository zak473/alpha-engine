...
Features are computed live from CoreMatch + BasketballTeamMatchStats using
the basketball feature_engineering module (no feat_basketball_match table).
...
def _load_training_data(session) -> tuple[np.ndarray, np.ndarray, list[str]]:
    """
    Load all finished basketball CoreMatch rows where outcome is known.
...
def train(version: Optional[str] = None) -> str:
    ...
    Returns the model name (e.g. 'basketball_lr_v1').
    """
    ...
    try:
        ...
        # Deactivate any existing live models for basketball
