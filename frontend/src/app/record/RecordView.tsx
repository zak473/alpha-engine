...
import { Trash2, TrendingUp, Clock, CheckCircle2, XCircle, MinusCircle, Plus, X, Search, ClipboardList } from "lucide-react";
...
function PickRow({ pick, onRemove }: { pick: PickOut; onRemove: () => void }) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-[14px] border border-white/[0.06] bg-white/[0.02] px-4 py-3 transition-colors hover:border-white/[0.10] hover:bg-white/[0.04]">
      {/* Sport dot */}
...
function TrackPickModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-[24px] border border-white/[0.08] bg-[linear-gradient(180deg,#0d1f18,#091510)] p-6 shadow-[0_32px_80px_rgba(0,0,0,0.5)]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38">Sport</label>
          ...
          {/* Match search */}
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38">Match</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-[10px] border border-white/[0.08] bg-white/[0.04] py-2 pl-8 pr-3 text-[13px] text-white placeholder-white/25 outline-none focus:border-emerald-400/30 focus:bg-white/[0.06] transition-colors"
              />
            </div>
            {loading && <p className="text-[11px] text-white/30">Searching…</p>}
            ...
            {selectedMatch && (
              <p className="text-[11px] text-emerald-400">✓ {selectedMatch.label}</p>
            )}
          </div>

          {/* Selection */}
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38">Selection</label>
            <div className="flex gap-2">
              {["Home", "Draw", "Away"].map((s) => (
                <button key={s} type="button"
                        onClick={() => setSelection(s)}
                        className={cn(
                          "flex-1 rounded-[10px] border py-1.5 text-[12px] font-semibold transition-all",
                          selection === s
                            ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300"
                            : "border-white/[0.08] bg-white/[0.03] text-white/45 hover:border-white/[0.14] hover:text-white/70"
                        )}>
          ...
          <div className="flex gap-3">
            <div className="flex flex-1 flex-col gap-2">
              <label className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38">Odds</label>
              <input type="number" step="0.01" min="1" value={odds}
                     onChange={(e) => setOdds(e.target.value)}
                     placeholder="2.10"
                     className="w-full rounded-[10px] border border-white/[0.08] bg-white/[0.04] px-3 py-2 text-[13px] text-white placeholder-white/25 outline-none focus:border-emerald-400/30 transition-colors"
                     required />
            </div>
            <div className="flex flex-1 flex-col gap-2">
              <label className="text-[11px] font-semibold uppercase tracking-[0.12em] text-white/38">Edge % <span className="normal-case text-white/25">(optional)</span></label>
              <input type="number" step="0.1" value={edge}
