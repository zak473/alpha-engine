              ...
              <div className="flex flex-col gap-1.5">
                {top.map((d) => (
                  <div key={d.score} className="flex items-center gap-3">
                    <span className="font-mono text-sm font-semibold w-10 text-center" style={{ color: C.textPrimary }}>{d.score}</span>
                    <div className="flex-1 h-4 rounded-lg overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
                      <div className="h-full rounded-lg" style={{ width: `${(d.probability / maxProb) * 100}%`, background: `${C.emerald}60` }} />
                    </div>
                    <span className="text-[11px] font-mono w-10 text-right" style={{ color: C.textSubtle }}>{(d.probability * 100).toFixed(1)}%</span>
                  </div>
                ))}
              </div>
            </Panel>
